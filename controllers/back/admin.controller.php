...
class AdminController{
    ...
    public function connexionBenevole(){
        if(!empty($_POST['login']) && !empty($_POST['password'])){
            $login = Securite::secureHTML($_POST['login']);
            $password = Securite::secureHTML($_POST['password']);
            if($this->adminManager->isBenevoleConnexionValid($login,$password)){
                $_SESSION['access'] = "admin";
                header('Location: '.URL."back/benevole");
            } else {
                header('Location: '.URL."back/login");
            }
        }
    ...
    public function getAccueilBenevole(){
        require "views/accueilBenevole.view.php";
    }
}